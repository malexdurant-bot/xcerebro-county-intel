"""
Maricopa County Superior Court Civil docket adapter — Phase 2 (Playwright).
...
Portal structure (confirmed 2026-06-26):
  ...
  - Two columns per result: Case Number + Party/Business Name
  - Full case details (filing date, type, plaintiff/defendant) are on caseInfo.asp

Supports four modes:
  --last-name / --first-name   : plaintiff/defendant individual name search
  --business-name              : business/entity name search
  --case-number                : direct case number lookup (repeatable)
  --probe-fields               : urllib GET to print discovered form field names

Lead-relevant case types: FC (foreclosure), CV (civil), LP (lis pendens),
MC (miscellaneous civil), SC (small claims).

Requires: pip install playwright && playwright install chromium

The adapter writes to data/raw/superior_court_civil.jsonl via the
...
DETAIL_BASE_URL = (
    "https://www.superiorcourt.maricopa.gov/docket/civilcourtcases/caseInfo.asp"
)
...
LEAD_RELEVANT_CASE_TYPES = frozenset(["CV", "FC", "LP", "MC", "SC"])
...
_FIELD_MAP: dict[str, str] = {
    ...
    "search_type_field": "rbSearchType",
}
...
def _raw_record_id(case_number: str) -> str:
    key = "|".join(["maricopa_civil", (case_number or "").strip().upper()])
...
# Container: #tblForms, each result: .row a[href*="caseInfo"]
# ---------------------------------------------------------------------------
...
def _pw_extract_results(
    ...
    for row in rows_data:
        ...
        raw_payload: dict = {
            ...
            "plaintiff": None,   # not determinable from search results listing
            "defendant": None,
            "filing_date": None,
            "case_type": None,
            "case_status": None,
...
def _pw_search_by_name(
    ...
    *,
    ...
    for fname in (_FIELD_MAP["business_name"], _FIELD_MAP["case_number"]):
...
def _pw_search_by_business_name(
    page,
    business_name: str,
    *,
    max_features: Optional[int] = None,
    ...
    for fname in (_FIELD_MAP["last_name"], _FIELD_MAP["first_name"], _FIELD_MAP["case_number"]):
        el = page.query_selector(f'[name="{fname}"]')
        if el:
            el.fill("")
...
def _pw_search_by_case_number(
    ...
    *,
    ...
    for fname in (_FIELD_MAP["last_name"], _FIELD_MAP["first_name"], _FIELD_MAP["business_name"]):
        el = page.query_selector(f'[name="{fname}"]')
        if el:
            el.fill("")
...
def run(
    *,
    ...
    if fetch_fn is not None:
        warnings.warn(
            "superior_court_civil: fetch_fn ignored in search mode — Playwright only.",
            stacklevel=2,
        )
    ...
    with sync_playwright() as pw:
        ...
        if business_name:
            try:
                records = _pw_search_by_business_name(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Maricopa County Superior Court Civil docket scraper (Playwright). "
            "Search by party name, business name, or case number and write "
            "results to JSONL. Targets FC (foreclosure), CV (civil), LP (lis pendens). "
            "Requires: pip install playwright && playwright install chromium."
        )
    )
    parser.add_argument("--last-name", default="",
                        help="Plaintiff/defendant last name for individual party search.")
    parser.add_argument("--first-name", default="",
                        help="Plaintiff/defendant first name for individual party search.")
    parser.add_argument("--business-name", default="",
                        help="Business or entity name for business search mode.")
    parser.add_argument("--case-number", action="append", dest="case_numbers", default=[],
                        metavar="CASE_NO",
                        help="Case number to look up directly (repeatable, e.g. CV2026-001234).")
    ...
    parser.add_argument("--out", default=None,
                        help="Output JSONL path. Default: data/raw/superior_court_civil.jsonl")
