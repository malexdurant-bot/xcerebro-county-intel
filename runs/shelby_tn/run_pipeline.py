...
from build_config import (  # noqa: E402
    ...
    TAX_SALE_JSONL, PARCEL_CACHE_PATH, OUTPUT_DIR,
)
...
def run_pipeline(
    ...
    if scrape:
        ...
        print(f"\n[ERROR] {TAX_SALE_JSONL} not found. Run with --scrape first.")
        sys.exit(1)
    ...
    if not tax_sale_records:
        print("  No active records — nothing to do.")
    ...
    # --- Build raw events from tax sale ------------------------------------
    print(f"\n[ADAPT] Converting {len(tax_sale_records)} tax sale records to raw events...")
    ...
    # Combine all sources (Phase 1: tax sale only)
    all_raw_events: list[dict] = tax_raw_events
